...
You MUST use the `Agent` tool to delegate this task. Do NOT answer as the go-build-resolver yourself.

Steps:
1. Load the Agent tool via `ToolSearch` if not yet available (`select:Agent`)
2. Call `Agent` with `subagent_type: "go-build-resolver"` and pass the user's full request as the prompt
