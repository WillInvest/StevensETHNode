...
2. **Manual**: Copy only this skill to `/home/stevensbc/stevens-blockchain/.claude/skills/configure-ecc/SKILL.md`, then activate by saying "configure ecc"
...
Options:
  - "User-level (/home/stevensbc/stevens-blockchain/.claude/)" — "Applies to all your Claude Code projects"
  - "Project-level (.claude/)" — "Applies only to the current project"
  - "Both" — "Common/shared items user-level, project-specific items project-level"
```

Store the choice as `INSTALL_LEVEL`. Set the target directory:
- User-level: `TARGET=/home/stevensbc/stevens-blockchain/.claude`
- Project-level: `TARGET=.claude` (relative to current project root)
- Both: `TARGET_USER=/home/stevensbc/stevens-blockchain/.claude`, `TARGET_PROJECT=.claude`
...
```bash
grep -rn "/home/stevensbc/stevens-blockchain/.claude/" $TARGET/skills/ $TARGET/rules/
grep -rn "../common/" $TARGET/rules/
grep -rn "skills/" $TARGET/skills/
```

**For project-level installs**, flag any references to `/home/stevensbc/stevens-blockchain/.claude/` paths:
- If a skill references `/home/stevensbc/stevens-blockchain/.claude/settings.json` — this is usually fine (settings are always user-level)
- If a skill references `/home/stevensbc/stevens-blockchain/.claude/skills/` or `/home/stevensbc/stevens-blockchain/.claude/rules/` — this may be broken if installed only at project level
...
- `continuous-learning-v2` references `/home/stevensbc/stevens-blockchain/.claude/homunculus/` directory
...
3. **Issue**: What's wrong (e.g., "references /home/stevensbc/stevens-blockchain/.claude/skills/python-patterns but python-patterns was not installed")
...
- For user-level: check `/home/stevensbc/stevens-blockchain/.claude/skills/<skill-name>/SKILL.md` exists
...
- Some skills assume `/home/stevensbc/stevens-blockchain/.claude/` paths. Run Step 4 verification to find and fix these.
- For `continuous-learning-v2`, the `/home/stevensbc/stevens-blockchain/.claude/homunculus/` directory is always user-level — this is expected and not an error.
